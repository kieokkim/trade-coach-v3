...
### TC-D13: v3.0 A+ Setup Memory 잠정 보류
...
**장기 수정:** TC-D14와 연계하여 캔들 기반 추론으로 완전 대체 (v2.2)

---

### TC-D17: A+ 채점 기준 ICT 이론 기반으로 재설계
...
### TC-D19: coaching_nodes.py direction 미추가
...
### TC-D30: A+ 채점 Eval 검증 - look-ahead bias 발견 및 수정
...
### TC-D31: 멀티모델 비교 평가 - Groq llama-3.3-70b 채택 검토
...
### TC-D32: Bybit API 권한 분리 - read-only 강제
...
### TC-D34: 거래소 추상화 레이어 + Upbit 지원 추가
...
### TC-D36: Observability - Langfuse 연동
**결정:** entry_reason_node, coaching_judge_node의 LLM 호출에 Langfuse trace 추가. 키 미설정 시 조용히 비활성화되어 기존 동작 영향 없음.

**목적:** 운영 중 LLM 비용/지연시간 추적, 멀티모델 비교(TC-D31) 결과를 실제 프로덕션 데이터로 지속 검증할 수 있는 기반 마련.
...
### TC-D38: 제품 스펙트럼 확정 — 행동 교정 축 / 자동매매 배제 / 코치형
...
### TC-D39: 2단계 엣지케이스 감사 — 구조적 결함 4건 확정
...
degradation 점검 대상에 넣어야 한다는 SalesCoach TC-D27의 교훈이 여기서도
그대로 재현됨.

※ 이 감사 이후 TC-D37(샘플모드 우선순위 수정)이 new_data_check_node/
bybit_fetch_node를 이미 수정했음을 확인. 결함1/9(실패=데이터없음=정상 혼동)와
TC-D37은 같은 함수를 다루지만 다른 버그 — TC-D37은 sample_mode가
API키 존재로 오버라이드되던 문제, 결함1/9는 dummy candle 자동대체가 실패/
빈데이터를 구분 안 하는 문제. TC-D37 이후에도 결함1/9가 남아있는지는
STEP1에서 재확인 필요.

---

### TC-D40: 분석철학 명명 — "ICT 룰 기반 행동 트레이딩 애널리틱스 · Unsupervised"
...
TradeCoach를 같은 틀로 부를 수 없음 — TC-D39에서 확인된 대로 A+ "5기준"이
...
### TC-D41: 차별점 재검토 — "저N 진단 가능성"은 차별점이 아니라 가설
...
- 그러나 이것이 경쟁력이 되려면 두 조건 필요, 둘 다 현재 미충족:
  ① ICT 이론 자체의 타당성이 검증돼야 함 (현재 사람검증 전무 — TC-D40)
  ② 저N에서 실제로 안전하게 작동해야 함 (현재 A+ 5기준 중 1개가 테이블 부재로
     항상 통과 — TC-D39 결함 2. "거래 1건도 안전하게 진단한다"는 주장이
     지금 코드로는 성립 안 함)
...
### TC-D42: 사람검증 부재 — 채우지 않고 재설계로 해결

**배경:** TC-D40에서 확인된 "사람검증 부재"를 (A)부재를 채운다 vs (B)부재가
...
**구현 방향(TC-D43에서 UI로 실체화):** 제품 UI에 "이 층은 코드검증됨 / 이 층은
...
### TC-D43: 복기뷰어 UI 재설계 — 캔들 표준색 + 존 무채색 + rule/LLM 시각분리
...
### TC-D44: 스코프 혼동 발견 — 계좌 전체 vs 거래 1건이 별개 그래프

**배경:** 사이드바 "약점"·"AI 코칭"이 한 거래의 것인지 계좌 전체의 것인지
불명확하다는 사용자 지적.

**발견:** TC-D39의 파이프라인 매핑으로 답이 나옴 — weakness_detect는 메인
...
구분은 나중에 층 분리(TC-D42)와도 연결 — 계좌 전체 진단과 거래별 진단이
다른 검증 지위를 가질 수 있음.

---

### TC-D45: 스택 판단 — 프레임워크 전환 불필요
...
### TC-D46: STEP1 진단 확정 — 결함1/9 방향 확정, 결함4/10 재설계 필요, 존 렌더링 원인 정정

**배경:** TC-D39(2단계 감사)에서 발견한 3개 항목의 수정 방향을 확정하기 위한
조사 세션(코드 수정 없음).

**결과:**
1. 결함1/9(get_candles): TC-D37과 무관 확인(거래 조회 경로 vs 캔들 조회
   경로, 서로 다른 함수). 수정방향 확정 — market/bybit_client.py::fetch_candles가
   예외 시 [] 대신 실패 신호 반환, market/candles.py::get_candles가 실패/빈데이터
   분기, 더미 대체는 sample_mode 전용으로 제한, api/main.py의 404 분기를 실제
   도달 가능하게.
2. 결함4/10(journal_entries): 재설계 필요로 격상. "journal_entries"라는 이름이
   메인그래프 state 필드(인메모리)와 entry_reason_node가 SQL로 쿼리하는 테이블명을
   동시에 가리키는 이름 충돌이었음 확인. 스키마 설계서(v2.1)에 애초에 이 테이블
   자체가 없음 — "구현 누락"이 아니라 "설계에 없던 유령 테이블". 단순 CREATE
   TABLE로 해결 안 됨. (a)journal_write_node가 실제 SQL 테이블에도 저장하도록
   확장 vs (b)기존 trade_tags/trade_history로 손절규율 판정 재설계, 둘 중 택1이
   선행 결정 필요 — 별도 세션으로 분리.
3. 존 렌더링 버그: 당초 "백엔드가 발생시각을 안 준다"는 가설 기각 —
   ict/fvg_detector.py·ob_detector.py·api/main.py·pages/3_main.py 전부 timestamp를
   정확히 쓰고 있음(프론트 앵커링도 2026-05-15부터 이미 구현). 실제 원인은
   탐지된 존 전부(FVG9+OB20)를 동일 투명도로 차트 우측 끝까지 그려서 겹쳐
   보이는 것 — 필터링·명암 차등 부재. 프론트 단일 파일(pages/3_main.py) 수정으로
   해결, 백엔드 무관.

**판단:** 수정 세션 순서 재확정 — A(fallback try/except)→B(실패/빈데이터
구분)→존 렌더링 필터링(A/B와 같은 세션, 백엔드 무관이라 공짜 추가작업)까지 한
세션. C(손절규율)는 설계 결정(위 a/b) 선행 필요해 별도 세션으로 분리.

**교훈:** 같은 이름이 서로 다른 두 실체(인메모리 state 필드 vs SQL 테이블)를
가리키는 경우, 이름만 보고 "테이블이 있다/없다"를 판단하면 안 됨 — 그래프
경계(메인 vs 복기 플로우)를 항상 먼저 확인해야 한다는 게 TC-D39에서도
나왔던 교훈의 재확인.
...
**결정:** MVP = 3단계 완주(사람검증으로 순환논리 끊기) + 목업 v7 기반
Streamlit 프론트 실구현 + 실거래 데이터 1회 이상 시험가동. ICT를 다른
rule-based 스타일로 교체 가능한지 탐색은 명시적으로 post-MVP.

**비고:** 합의는 2026-07에 됐으나 커밋이 누락돼 뒤늦게 기록.

---

### TC-D48: 코칭 제품 보류 (폐기 아님)

**결정:** TradeCoach 코칭 제품을 보류하고, ICT 자동매매 프로젝트 쪽에서
모니터링 에이전트를 별도 제품으로 신규 착수.

**근거:**
1. 봇이 실행 주체가 되면 코칭 대상의 심리 편향이 0이다 — 제품 정의(행동·심리
   코칭)의 적용 대상 자체가 사라진다.
2. 봇 진입 조건이 고정(sweep→FVG∩OB + 4h bias + 롱 전용)이라 거래가 전부 동일
   셋업 유형이다. A+ 스코어링의 점수 분산이 안 생겨 A+/非A+ 성과 비교가
   성립하지 않는다.
3. Phase B(A+ 검증)는 사용자 거래 데이터가 필요하고 Phase D(외부 배포)는
   반드시 B 이후다. 사용자는 D에서 온다 — B←D←B 데드락.
