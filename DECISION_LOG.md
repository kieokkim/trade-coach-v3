...
### Decision 46: STEP1 진단 확정 — 결함1/9 방향 확정, 결함4/10 재설계 필요, 존 렌더링 원인 정정

**배경:** Decision 39(2단계 감사)에서 발견한 3개 항목의 수정 방향을 확정하기 위한
조사 세션(코드 수정 없음).

**결과:**
1. 결함1/9(get_candles): Decision 37과 무관 확인(거래 조회 경로 vs 캔들 조회
...
경계(메인 vs 복기 플로우)를 항상 먼저 확인해야 한다는 게 Decision 39에서도
나왔던 교훈의 재확인.
